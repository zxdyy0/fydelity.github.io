...
import { pickAndScanFolder, rescanFolder, canRescan, computeStorageStats, supportsFileSystemAccess } from './file-source.js';
...
async function runFolderPick() {
  ...
  try {
    ...
    console.error(err);
    onbText.textContent = 'Could not read that folder — try again';
    onbCta.style.display = '';
    onbNote.style.display = '';
    onbScan.classList.remove('show');
  }
  ...
  if (tracks.length === 0) {
    trackListEl.innerHTML = `
      <div class="lib-empty">
        ...
        <div class="lib-empty-sub">Go to Settings → Re-scan, or reopen the app to point Fydelity at a folder.</div>
      </div>`;
    return;
  }
