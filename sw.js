...
// Large, decode-only assets: cached on first real use, not on install.
const RUNTIME_CACHE_PATTERNS = [/\/vendor\/ffmpeg\/core\//];
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.origin !== self.location.origin) return; // fonts CDN etc. — let the browser handle those normally

  const isRuntimeAsset = RUNTIME_CACHE_PATTERNS.some((re) => re.test(url.pathname));

  if (isRuntimeAsset) {
    // cache-first, but fetch-and-cache on miss — this is the "first APE/FLAC
    // decode downloads it once, every decode after that (even offline) is instant" path
    event.respondWith(
